...
def render_status_banner(report: dict[str, object]) -> bytes:
    ...
    content = "".join(
        (
            ...
            "<style>html{background:#edf0f7;color:#17233b;}",
            "body{margin:0;min-width:0;}p{box-sizing:border-box;font-size:13px;",
            "line-height:1.5;max-width:1248px;margin:0 auto;padding:10px 0;}",
            "strong{font-weight:600}time{color:#57647a;white-space:nowrap}",
            ".status-detail{display:none}",
            "a{color:inherit;text-underline-offset:2px}",
            "a:focus{outline:3px solid #254cdb;outline-offset:3px}",
            "@media(max-width:1328px){p{padding-inline:24px}}",
            "</style></head>\n",
            f'<body><p><strong>{escape(lead)}</strong> <span class="status-detail">',
            f"{escape(detail)} </span>",
            '<a target="_top" href="../v1/status.json">Status record</a> · ',
            f'<time datetime="{escape(generated_at)}">Checked {escape(generated_at[:10])}</time>',
            "</p></body></html>\n",
        )
    )
